...
1.  Simple property values by default are converted to String, Int, or Double
1.  Property values which follow the correct format may be decoded into URL, Data, or Date.
1.  Any property value may be assumed to be Optional
1.  Square bracket [] values are converted to Array
1.  Curly brackets {} values are convert to Dictionary or a custom type.
1.  All types which need to be decoded need to implement the Decodable protocol. Likewise, types which need to be encoded need to implement the Encodable protocol. If the type needs to be both decoded and encoded can simply implement Codable. 
...
In addition, if KeyDecodingStrategy.convertFromSnakeCase doesn’t quite fit with your JSON keys, Swift does allow the use of custom strategies. In other words, you can supply a closure for converting a CodingKey array to single CodingKey result.Last but not least, if none of this strategies work for converting a key to a property name you can always supply a custom CodingKeys enumeration. In short, you want to use the simplest and most consistent strategy for mapping property names or JSON keys.
