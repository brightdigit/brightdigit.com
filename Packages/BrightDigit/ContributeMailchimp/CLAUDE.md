...
ContributeMailchimp is a Swift library (SPM package, product `ContributeMailchimp`) that turns Mailchimp newsletters into Markdown files with YAML front matter, using the `Contribute` pipeline. It is consumed as a library; there is no executable target.
...
Depends on the sibling `Contribute` package (the generic source→Markdown pipeline) and `Spinetail` (the Mailchimp API client), both resolved in-place via relative paths in the monorepo.
