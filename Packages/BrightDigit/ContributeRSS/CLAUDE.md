...
ContributeRSS is a Swift library (SPM package, product `ContributeRSS`) that turns RSS/Atom feeds into Markdown files with YAML front matter, using the `Contribute` pipeline. It is consumed as a library; there is no executable target.
...
Depends on the sibling `Contribute` package (the generic source→Markdown pipeline) and `SyndiKit` (RSS/Atom feed parsing), both resolved in-place via relative paths in the monorepo.
