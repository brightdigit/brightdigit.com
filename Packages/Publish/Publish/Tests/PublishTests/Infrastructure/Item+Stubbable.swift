...
extension Item: Stubbable where Site == WebsiteStub.WithoutItemMetadata {
  ...
  internal static func stub(withSectionID sectionID: WebsiteStub.SectionID) -> Self {
    stub(withPath: Path(.unique()), sectionID: sectionID)
  }

  internal static func stub(withPath path: Path, sectionID: WebsiteStub.SectionID) -> Self {
    Item(
      path: path,
      sectionID: sectionID,
      metadata: Site.ItemMetadata(),
