...
internal final class HTMLTests: XCTestCase {
  ...
  internal func testPageDescription() {
  ...
  internal func testSocialImageMetadata() {
    let html = HTML(
      .head(
  ...
  internal func testResponsiveViewport() {
  ...
  internal func testStaticViewport() {
  ...
  internal func testViewportFit() {
    let html = HTML(.head(.viewport(.accordingToDevice, fit: .cover)))
    // The expected value is a single line of rendered HTML that must match
    // the renderer's output exactly, so it cannot be wrapped.
  ...
  internal func testLinkWithHrefLang() {
    let html = HTML(
      .head(
        .link(
          .rel(.alternate),
  ...
  internal func testAppleTouchIconLink() {
    let html = HTML(
      .head(
        .link(
          .rel(.appleTouchIcon),
  ...
  internal func testCrossoriginLinkEnabled() {
    let html = HTML(
      .head(
        .link(
          .rel(.preconnect),
  ...
  internal func testCrossoriginLinkDisabled() {
    let html = HTML(
      .head(
        .link(
          .rel(.preconnect),
  ...
  internal func testManifestLink() {
    let html = HTML(
      .head(
        .link(
          .rel(.manifest),
  ...
  internal func testMaskIconLink() {
    let html = HTML(
      .head(
        .link(
          .rel(.maskIcon),
  ...
  internal func testTitleAttribute() {
    let html = HTML(
      .head(
        .link(
          .rel(.alternate),
  ...
  internal func testDescriptionList() {
    let html = HTML(
      .body(
        .dl(
          .dt("Term"),
  ...
  internal func testDescriptionListWithDiv() {
    let html = HTML(
      .body(
        .dl(
          .div(
  ...
  internal func testTextDirectionalityLeftToRight() {
    let html = HTML(
      .body(
  ...
  internal func testTextDirectionalityRightToLeft() {
    let html = HTML(
      .body(
  ...
  internal func testTextDirectionalityAuto() {
    let html = HTML(
      .body(
        .h1(.dir(.auto), "Text")
      )
  ...
  internal func testInputDirectionalityAuto() {
    let html = HTML(
      .body(
        .input(.dir(.auto))
      )
  ...
  internal func testTextAreaDirectionalityLeftToRight() {
    let html = HTML(
      .body(
        .textarea(.dir(.auto))
      )
  ...
  internal func testAnchors() throws {
    let html = try HTML(
      .body(
  ...
  internal func testTableGroupingSemantics() {
    let html = HTML(
      .body(
        .table(
          .thead(
  ...
  internal func testEmbeddedObject() {
    let html = HTML(
      .body(
        .embed(
          .src("url"),
  ...
  internal func testFormContentType() {
    let html = HTML(
      .body(
  ...
  internal func testFormNoValidate() {
    let html = HTML(
      .body(
        .form(.novalidate())
      )
  ...
  internal func testFormWithBodyNodes() {
    let html = HTML(
      .body(
        .form(
          .method(.post),
  ...
  internal func testAbbreviation() {
    let html = HTML(
      .body(
  ...
  internal func testAccessibilityControls() {
  ...
  internal func testSpellcheckAttribute() {
    let html = HTML(
      .body(
        .spellcheck(true),
        .form(
  ...
  internal func testSubresourceIntegrity() {
    let html = HTML(
      .head(
        .script(.src("file.js"), .integrity("sha384-fakeHash")),
        .link(
          .rel(.stylesheet), .href("styles.css"), .type("text/css"), .integrity("sha512-fakeHash")
        ),
