...
internal struct MarkdownVisitor {
  ...
  internal struct ParsedFragment {
    ...
    internal var rawString: Substring
  }
  ...
  internal init(source: String, urls: NamedURLCollection) {
    self.source = source
    self.urls = urls
  }
  ...
  internal func makeFragments(from document: Document) -> [ParsedFragment] {
    document.blockChildren.compactMap { block in
      guard let fragment = fragment(for: block) else {
        return nil
      }
...
extension MarkdownVisitor {
  ...
  fileprivate func rawString(for markup: Markup) -> Substring {
    guard let range = markup.range else {
      return ""
    }
    let lower = source.lineColumnIndex(
      line: range.lowerBound.line,
      column: range.lowerBound.column
    )
    let upper = source.lineColumnIndex(
  ...
  // Inherently branchy: one case per swift-markdown block type.
  // swiftlint:disable:next cyclomatic_complexity
  fileprivate func fragment(for markup: Markup) -> Fragment? {
    switch markup {
  ...
  fileprivate func renderBlocks<S: Sequence>(_ blocks: S) -> String where S.Element == BlockMarkup {
  ...
  fileprivate func renderInline<S: Sequence>(_ inlines: S) -> String
  ...
  fileprivate func makeCodeBlock(_ codeBlock: Markdown::CodeBlock) -> CodeBlock {
    ...
    return CodeBlock(
      language: Substring(language),
      code: HTMLEscaping.escape(codeBlock.code)
    )
  }

  fileprivate func makeList(items: [ListItem], kind: List.Kind) -> List {
  ...
  fileprivate func makeTable(_ table: Markdown::Table) -> Table {
    ...
    return Table(
      renderedHeader: Array(header),
      renderedRows: Array(bodyRows),
      columnAlignments: alignments
    )
  }

  fileprivate func format(_ markup: Markup) -> String {
  ...
  fileprivate func trimTrailingNewlines(_ string: String) -> String {
