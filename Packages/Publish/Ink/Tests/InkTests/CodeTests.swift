...
internal final class CodeTests: XCTestCase {
  internal func testInlineCode() {
    let html = MarkdownParser().html(from: "Hello `inline.code()`")
    XCTAssertEqual(html, "<p>Hello <code>inline.code()</code></p>")
  }

  internal func testCodeBlockWithJustBackticks() {
  ...
  internal func testCodeBlockWithBackticksAndLabel() {
    let html = MarkdownParser().html(
      from: """
        ```swift
        code()
  ...
  internal func testCodeBlockWithBackticksAndLabelNeedingTrimming() {
  ...
  internal func testCodeBlockManyBackticks() {
    // there are 2 spaces after the swift label that need trimming too
    let html = MarkdownParser().html(
      from: """
  ...
  internal func testEncodingSpecialCharactersWithinCodeBlock() {
    let html = MarkdownParser().html(
      from: """
        ```swift
  ...
  internal func testIgnoringFormattingWithinCodeBlock() {
    let html = MarkdownParser().html(
      from: """
        ```
        # Not A Header
...
extension CodeTests {
  internal static var allTests: Linux.TestList<CodeTests> {
    [
      ...
      (
        "testCodeBlockWithBackticksAndLabelNeedingTrimming",
        testCodeBlockWithBackticksAndLabelNeedingTrimming
      ),
