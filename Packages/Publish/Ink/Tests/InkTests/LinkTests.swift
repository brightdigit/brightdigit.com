...
internal final class LinkTests: XCTestCase {
  internal func testLinkWithURL() {
    let html = MarkdownParser().html(from: "[Title](url)")
    XCTAssertEqual(html, #"<p><a href="url">Title</a></p>"#)
  }

  internal func testLinkWithReference() {
  ...
  internal func testCaseMismatchedLinkWithReference() {
    let html = MarkdownParser().html(
      from: """
        [Title][Foo]
        [Title][αγω]
  ...
  internal func testNumericLinkWithReference() {
    let html = MarkdownParser().html(
      from: """
        [1][1]
...
extension LinkTests {
  internal static var allTests: Linux.TestList<LinkTests> {
    [
      ...
      (
        "testLinkWithParenthesisAndClosingParenthesisInContent",
        testLinkWithParenthesisAndClosingParenthesisInContent
      ),
