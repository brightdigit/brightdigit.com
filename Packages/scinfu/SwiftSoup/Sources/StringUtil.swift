...
//    }
    ...
    // brightdigit.com vendored patch: dropped `@inline(__always)` here. Forcing this unsafe
    // byte-loop worker to inline into `Element.appendNormalisedText` (and the *Tracking variants)
    // builds a function the Swift 6.4 nightly optimizer crashes on during `swift build -c release`.
    // Letting the compiler decide inlining keeps full `-O` optimization without the crash. Restore
    // the attribute once the toolchain stabilises / upstream fixes it. See CI run 27729200662.
