...
internal extension Client where APIType == YouTube.API {
    ...
    playlistItems(fromRequest: request) { result in
      ...
      let requests = playlistItems
        ...
        .chunked(by: 50).map { ids in
          Videos.YoutubeVideosList.Request(
            fields: "items(id,contentDetails/duration,snippet/title,snippet/description,snippet/publishedAt,snippet/thumbnails)",
            key: request.apiKey,
  ...
  private func playlistItems(
    fromRequest request: YouTubePlaylistRequest,
    withAccumulatingResult initialResult: AccumulatingPlaylist? = nil,
    _ completed: @escaping (Result<[PlaylistItem], ClientError>) -> Void
  ) {
    var nextResult = initialResult?.items ?? .init()

    self.request(
      PlaylistItems.YoutubePlaylistItemsList.Request(
        fields: "items/snippet/resourceId/videoId,nextPageToken",
        key: request.apiKey,
        part: ["snippet"],
        maxResults: 50,
        pageToken: initialResult?.nextPageToken,
