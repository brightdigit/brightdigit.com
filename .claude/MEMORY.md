...
- [README is blog-post source](.claude/memory/readme-blog-post-source.md) — root README doubles as blog-post material; keep it narrative-rich with real numbers from PRs/package repos when updating
- [Wave 1 hygiene pass + Contribute 6.4](.claude/memory/wave1-hygiene-pass.md) — 2026-07-23/24: 7 Wave 1 repos to Wave 0 standard; un-deprecated YouTube/Mailchimp + tests; Contribute→6.4 (#19) exposed a real ContributeWordPress data race (#18, must land together); Plot/Files/Ink missing CLAUDE_CODE_OAUTH_TOKEN secret
- [CI netlify-cli decoupled from build image](.claude/memory/ci-netlify-deploy-image-decoupling.md) — 2026-08-05→17 deploy outage: `publish-xml:6.4` rebuilt without its netlify-cli layer → `netlify: not found`; now installed+cached in the workflow; rejected alternatives (netlify/actions, netlify docker images) recorded
- [automate-content content poll](AGENTS.md) — the 6-hourly poll runs `import mailchimp` (frozen #001-#117 archive), `import podcast`, and `import buttondown` (live newsletters, #118+); deliberately all-or-nothing, no `continue-on-error`
- [Claude Design redesign brief](.claude/memory/claude-design-redesign-brief.md) — ready-to-paste prompt + attachment checklist for the post-phases site redesign; artboard list, brand tokens, Tailwind-v4/TailwindKit + AI-CITE constraints, screenshot script
