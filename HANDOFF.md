...
- [x] **macOS + Windows CI legs added** to all 5 standalone subrepos (ButtondownKit/Spinetail/SwiftTube/Contribute/SyndiKit). macOS runs on the **self-hosted runner with `/Applications/Xcode-beta.app`** (Swift 6.4); Windows on a swift.org nightly snapshot (`6.4.x-DEVELOPMENT-SNAPSHOT-2026-06-01-a`, gated to full-matrix runs). SyndiKit also gains the iOS/watchOS/tvOS suite (`build-macos-platforms`, released Xcode_26.4 — not nightly 6.4). Windows is `continue-on-error` on the OpenAPI repos + Contribute (unverified deps); blocking on SyndiKit. Contribute & SyndiKit migrated 6.3→nightly-6.4. **Bump the Windows snapshot id periodically** (swift.org GCs old dev snapshots).
- [ ] **WASM + Android CI legs deferred** — no nightly 6.4 support yet. WASM blocked by swift-build ([brightdigit/swift-build#115](https://github.com/brightdigit/swift-build/issues/115): no input to override the auto-derived `-RELEASE` wasm SDK URL); Android blocked upstream (no nightly 6.4 Android SDK published). Deferred-leg comments in each workflow document re-adding them (job bodies in git history of `packages.yaml`). Contribute WASM is permanently N/A (Yams fails on the Musl/wasm SDK).
